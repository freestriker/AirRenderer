...
{
    FIBITMAP* texture = global.loadThread->GetResource<FIBITMAP>(loadCommand);
    glm::ivec2 size = glm::vec2(FreeImage_GetWidth(texture), FreeImage_GetHeight(texture));
    glm::vec2 pos = Wrap(uv);
    Color c = Interpolate(uv, texture, size);
    return c;
}
...
Texture::Texture(std::string filePath, InterpolationOption interpolationOption, WrapOption wrapOption, MipMapOption mipMapOption)
{
    ...
    //lc.path = filePath;
    //lc.processOption = LoadThread::ProcessOptions::TEXTURE;
...
Color Texture::Interpolate(glm::vec2& uv, void* texture, glm::ivec2& size)
{
    ...
    switch (this->interpolationOption)
    {
        ...
        {
            return GetColor(fxy, texture, size);
        }
        case InterpolationOption::BILINERA:
        {
            ...
            Color floorColor = GetColor(glm::vec2(x[0], y[0]), texture, size) * (1.0f - scale[0]) + GetColor(glm::vec2(x[1], y[0]), texture, size) * scale[0];
            Color ceilColor = GetColor(glm::vec2(x[0], y[1]), texture, size) * (1.0f - scale[0]) + GetColor(glm::vec2(x[1], y[1]), texture, size) * scale[0];
            return floorColor * (1.0f - scale[1]) + ceilColor * scale[1];
        }
    }
    throw "";
}
Color Texture::GetColor(glm::vec2 pos, void* texture, glm::ivec2& size)
{
    pos = glm::clamp(pos, fzero, glm::vec2(size));
    glm::ivec2 position = glm::clamp(glm::iround(pos), izero, size - ione);

    int bytespp = FreeImage_GetLine(static_cast<FIBITMAP*>(texture)) / size.x;
    BYTE* bits = FreeImage_GetScanLine(static_cast<FIBITMAP*>(texture), position.y);
